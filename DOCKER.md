...
## Build (no cache)
Ensure a clean install from the locked dependency set:
...
Notes:
- The Dockerfile installs dependencies with `pipenv install --deploy --system` using the checked-in `Pipfile.lock` for deterministic builds.
...
- Postgres: port 5438 exposed (for local tooling)
...
- Consider removing the `ports: 5438:5432` mapping on the db service for closed environments.
