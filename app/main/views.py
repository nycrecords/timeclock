...
def index():
    ...
    if not current_user.is_authenticated:  # Don't pass a form
        current_app.logger.error(
            "Anonymous user visited index. Redirecting to /auth/login..."
        )
    ...
    if form.validate_on_submit():
        if form.note.data is not None and len(form.note.data) > 60:
            ...
            current_app.logger.error(
                "{} submitted a note that exceeded 60 characters".format(
                    current_user.email
                )
            )
        else:
...
def all_history():
    ...
    if "first_date" not in session:
        current_app.logger.info(
            "'first_date' not found in session. Setting to defaults."
        )
    ...
    if "tag_input" not in session:
        current_app.logger.info(
            "'tag_input' not found in session. Setting to defaults."
        )
        session["tag_input"] = 0
    ...
    if request.referrer and "all_history" not in request.referrer:
        ...
        current_app.logger.info(
            "User is visiting from another page. Setting session['email'] to None"
        )
        session["email"] = None
    ...
    if form.validate_on_submit() and (
        form.submit.data
        or form.last_month.data
        or form.this_month.data
        or form.last_week.data
        or form.this_week.data
        or form.last_day.data
        or form.this_day.data
    ):
    ...
    if (
        deleteform.validate_on_submit()
        and request.form.get("event_id", None)
        and deleteform.delete.data
    ):
    ...
    if advtimesheetform.validate_on_submit() and advtimesheetform.gen_timesheets.data:
        ...
        return generate_timesheets(
            advtimesheetform.emails.data,
            advtimesheetform.start_date.data,
...
def history():
    ...
    if not current_user.validated:
        current_app.logger.info(
            "{} is not validated. Redirecting to /auth/change_password...".format(
                current_user.email
            )
        )
        return redirect(url_for("auth.change_password"))
...
def download():
    ...
    events = request.form.getlist(
        "event"
    )  # Gets event data from frontend - we can similarly pass in other data

    if not (check_total_clock_count(events)):
        current_app.logger.error(
            "Timesheet was generated with odd number of clock ins/outs {}".format(
                len(events)
            )
        )
        flash(
    ...
    if errors:
        ...
        current_app.logger.error(
            "Errors occurred while generating timesheet (end function download)."
...
def download_invoice():
    ...
    if errors:
        for error in errors:
            flash(error, "warning")
        current_app.logger.error(
            "Errors occurred while generating invoice (end function download_invoice)."
            " Redirecting to main.{}...".format(last_page)
        )
        return redirect(url_for("main." + last_page))
    ...
    if (
        ...
        flash(
            "User {} does not have a payrate. Maybe you meant to generate a timesheet instead.".format(
                session["email"]
            ),
            category="error",
        )
        flash(
            "If you believe this is an error, please contact a TimeClock administrator.",
            category="warning",
        )
        return redirect(url_for("main." + last_page))

    all_info = calculate_hours_worked(
        session["email"], session["first_date"], session["last_date"]
    )
    if not all_info:
        current_app.logger.error(
    ...
    return render_template(
        ...
        day_events_list=day_events_list,
        employee=u,
        total_hours=total_hours,
        total_earnings=total_earnings,
        time=datetime.now(),
        budget_code=u.budget_code,
        object_code=u.object_code,
...
def clear():
    ...
    current_app.logger.info(
        "User %s cleared their admin history filter." % current_user.email
    )
...
def user_clear():
    ...
    session.pop("tag_input", None)
    current_app.logger.info(
        "User %s cleared their history filter." % current_user.email
    )
...
def pay():
    ...
    if form.validate_on_submit():
        ...
        if not u:
            current_app.logger.error(
                "Tried creating pay for {}. A user with this email does not exist.".format(
                    form.email.data
                )
            )
            ...
            db.session.commit()
            current_app.logger.info(
                "Administrator {} created new pay rate for user {}".format(
    ...
    return render_template(
        "payments/create_payrate.html", form=form, pays=Pay.query.all()
    )
...
def request_timepunch():
    ...
    if form.validate_on_submit() and form.submit.data:
        if form.note.data is not None and len(form.note.data) > 60:
            ...
            flash(
                "You must have a supervisor to request a timepunch. If you believe a supervisor "
                "should be assigned to you, please contact the system administrator.",
                category="error",
            )
            current_app.logger.error(
            ...
            try:
                ...
                return redirect(url_for("main.request_timepunch"))
            past_type = (
                get_last_clock(current_user, datetime_obj).type
                if get_last_clock(current_user, datetime_obj)
                else None
            )

            if past_type == (form.punch_type.data == "In"):
                flash(
                    "Timepunch request failed: you would have two consecutive clocks of the same type",
                    "error",
                )
                return redirect(url_for("main.request_timepunch"))
            create_timepunch(form.punch_type.data, datetime_obj, form.note.data)
            flash(
                "Your timepunch request has been successfully submitted and is pending approval",
                category="success",
            )
        ...
        if not current_user.supervisor:
            ...
            v = Vacation(
                ...
                start=vacform.vac_start.data,
                end=vacform.vac_end.data,
                approved=False,
                pending=True,
            )
            db.session.add(v)
            db.session.commit()
            flash(
                "Your vacation request has been successfully submitted and is pending approval",
                "success",
            )
...
def review_timepunch():
    ...
    if filter_form.validate_on_submit and filter_form.filter.data:
        ...
        timepunch_query = get_timepunches_for_review(
            current_user.email, filter_form.email.data, filter_form.status.data
        )
    ...
    if form.validate_on_submit():
        ...
        if form.approve.data:
            ...
            current_app.logger.info(
                "{} approved timepunch with event_id {}".format(
                    current_user.email, request.form["event_id"]
                )
            )
        elif form.deny.data:
            ...
            current_app.logger.info(
                "{} denied timepunch with event_id {}".format(
                    current_user.email, request.form["event_id"]
                )
            )
...
def review_vacations():
    ...
    if filter_form.validate_on_submit and filter_form.filter.data:
        ...
        # Filter through timepunches based on user selections
        vacation_query = get_vacations_for_review(
            current_user.email, filter_form.email.data, filter_form.status.data
        )
    ...
    if form.validate_on_submit():
        ...
        if form.approve.data:
            ...
            current_app.logger.info(
                "{} approved vacation with event_id {}".format(
                    current_user.email, request.form["vacation_id"]
                )
            )
        elif form.deny.data:
            ...
            current_app.logger.info(
                "{} denied vacation with event_id {}".format(
                    current_user.email, request.form["vacation_id"]
                )
            )
