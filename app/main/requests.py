...
def get_timepunches_for_review(user_email, filter_by_email=None, status=None):
    ...
    if status == "Approved":
        timepunch_query = timepunch_query.filter(Event.approved == True).filter(
            Event.pending == False
        )
    elif status == "Unapproved":
        timepunch_query = timepunch_query.filter(Event.approved == False).filter(
            Event.pending == False
        )
...
def get_vacations_for_review(user_email, filter_by_email=None, status=None):
    ...
    # Filter by status if user provides a status
    if status == "Pending":
        vacation_query = vacation_query.filter(Vacation.pending == True).filter(
            Vacation.approved == False
        )
    if status == "Approved":
        vacation_query = vacation_query.filter(Vacation.approved == True).filter(
            Vacation.pending == False
        )
    elif status == "Unapproved":
        vacation_query = vacation_query.filter(Vacation.approved == False).filter(
            Vacation.pending == False
        )
        # else status == 'All', in which case we don't need to add anything to the filter
