...
def get_payrate_before_or_after(email_input, start, before_or_after):
    ...
    if user:
        ...
        current_app.logger.info(
            "Querying for most recent pay for user {}".format(email_input)
        )
        pay_query = Pay.query.filter(Pay.user_id == user.id)
        if before_or_after:
            results = (
                pay_query.filter(Pay.start <= start)
                .order_by(sqlalchemy.desc(Pay.start))
                .all()
            )
            p = results[0] if results else None
            # Get the most recent payment on/before the given date
        else:
            p = pay_query.filter(Pay.start >= start).order_by(Pay.start.asc()).first()
            # Get the first payment after the given date
        if not p:
            current_app.logger.error(
                "No pay for user {} exists before date {}".format(email_input, start)
            )
        current_app.logger.info(
            "Finished querying for most recent pay for user {}".format(email_input)
        )
    else:
        current_app.logger.error(
            "User with email {} was not found in database. Aborting...".format(
                email_input
            )
        )
        p = None
...
def calculate_hours_worked(email_input, start, end):
    ...
    all_info = {"days_list": [], "total_hours": 0, "total_earnings": 0}
    ...
    for x in range(0, len(events), 2):
        ...
        payrate_this_day = get_payrate_before_or_after(
            email_input, time_in.date(), True
        )
