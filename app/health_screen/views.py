...
from app.health_screen.forms import (
    HealthScreenForm,
    HealthScreenAdminForm,
    AddHealthScreenUserForm,
    EditHealthScreenUserForm,
)
from app.health_screen.utils import (
    process_health_screen_confirmation,
    generate_health_screen_export,
    generate_health_screen_daily_summary_export,
)
    ...
    if form.validate_on_submit():
        if (
            ...
            return render_template(
                "health_screen/health_screen_confirm.html", form=form
            )
...
def health_screen_filter():
    ...
    # Defaults
    results = (
        HealthScreenResults.query.filter(
            HealthScreenResults.date.cast(Date) == datetime.today().date()
        )
        .order_by(desc(HealthScreenResults.id))
        .all()
    )
    ...
    if form.validate_on_submit():
        ...
        search_results = (
            HealthScreenResults.query.filter(and_(*query_filters))
            .order_by(desc(HealthScreenResults.id))
            .all()
        )

        if "export" in request.form:
            health_screen_export = generate_health_screen_export(
                search_results, "health_screen_results.xlsx"
            )
            return send_file(
        ...
        return render_template(
            "health_screen/health_screen_filter.html", results=search_results, form=form
        )
    return render_template(
        "health_screen/health_screen_filter.html", results=results, form=form
    )
...
def health_screen_daily_summary():
    ...
    if "export" in request.form:
        health_screen_export = generate_health_screen_daily_summary_export(
            daily_results,
            filename="health_screen_results_{}.xlsx".format(
                date.strftime("%-m-%-d-%Y")
            ),
        )
        return send_file(
    ...
    return render_template(
        "health_screen/health_screen_daily_summary.html",
...
def add_health_screen_user():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(
            "New health screen user created.", category="success",
        )
...
def edit_health_screen_user(user_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(
            "User information edited.", category="success",
        )
        return redirect(url_for("health_screen.health_screen_users"))
    return render_template(
        "health_screen/edit_health_screen_user.html", form=form, user=user
    )
...
def remove_health_screen_user(user_id):
    ...
    db.session.commit()
    flash(
        "User {} Removed.".format(user_name), category="success",
    )
    return redirect(url_for("health_screen.health_screen_users"))
