...
def process_health_screen_confirmation(
    ...
    send_email(
        ...
        subject="(Report to Work: {report_to_work} - {date}) Health Screening Confirmation - {name}".format(
            report_to_work=health_screen.report_to_work, date=date, name=name
        ),
...
def generate_health_screen_confirmation(canvas_field, health_screen_results):
    ...
    canvas_field.drawString(
        70,
        LENGTH - 200,
        "1.   Employee confirms they have completed the entire health check questionnaire: {questionnaire_confirmation}".format(
            questionnaire_confirmation=health_screen_results.questionnaire_confirmation
        ),
    )
    canvas_field.drawString(
        70,
...
def generate_health_screen_export(results, filename):
    ...
    for result in results:
        ...
        elif not result.report_to_work == "Yes" and result.questionnaire_confirmation == "Yes":
...
def generate_health_screen_daily_summary_export(results, filename):
    ...
    for result in results:
        if result[3] == "Yes":
            ...
            worksheet.write(row, col + 4, "Yes", green)
        elif result[3] == "No" and result[2] == "Yes":
